...
void lcd_puts_eep(const unsigned char *eep){
    register unsigned char c;
    while ( (c = eeprom_read_byte(eep++)) ) {
        lcd_putc(c);
    }
}
#endif
...
void lcd_putc_cgram_eep(const unsigned char *eep, unsigned char _position){
	lcd_command((1<<LCD_CGRAM)+_position*8);
	register unsigned char c;
	register char x1;
    for (x1=0;x1<=7;x1++){
		(c = eeprom_read_byte(eep++));  
        lcd_putc(c);
	}
	lcd_clrscr();
}
